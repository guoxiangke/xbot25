...
class SelfMessageCheckInRoomAutoConfigTest extends TestCase
{
    ...
    /**
     * 测试：当全局 room_msg 关闭时，启用 check_in 应该自动启用 room_listen
     */
    public function test_check_in_auto_enables_room_listen_when_global_room_msg_disabled()
    {
        ...
        $this->assertNull($this->wechatBot->getMeta('room_msg_specials')[$roomWxid] ?? null);
        
        // 构造消息上下文
        $context = $this->createMessageContext($roomWxid, '/config check_in 1');
        ...
        $this->assertTrue($this->wechatBot->getMeta('room_msg_specials')[$roomWxid] ?? false);
    }

    /**
     * 测试：当全局 room_msg 开启时，启用 check_in 不应该自动设置 room_listen
     */
    public function test_check_in_does_not_auto_enable_room_listen_when_global_room_msg_enabled()
    {
     ...
     */
    public function test_check_in_does_not_override_existing_room_listen_config()
    {
        ...
        $this->wechatBot->setMeta('room_msg_specials', [$roomWxid => false]);
        ...
        $this->assertFalse($this->wechatBot->getMeta('room_msg_specials')[$roomWxid]);
    }

    /**
     * 测试：关闭 check_in 不应该影响 room_listen 配置
     */
    public function test_disabling_check_in_does_not_affect_room_listen()
    {
        ...
        // 构造消息上下文关闭 check_in
        $context = $this->createMessageContext($roomWxid, '/config check_in 0');
        ...
        $this->assertTrue($this->wechatBot->getMeta('room_msg_specials')[$roomWxid]);
    }
