...
class ConfigManager
{
    ...
    /**
     * 资源频道级开关（按 xlaravel handler / statistics.metric 命名，如 Febc、LyAudio）
     * 约定：meta key = keyword_resources_{channel}_enabled，默认启用（黑名单语义）
     */
    private function getResourceChannelKey(string $channel): string
    {
        return "keyword_resources_{$channel}_enabled";
    }

    /**
     * 判断某资源频道是否对当前 bot 启用（默认启用，未显式禁用即放行）
     */
    public function isResourceChannelEnabled(string $channel): bool
    {
        if ($channel === '') {
            return true;
        }

        return (bool) $this->wechatBot->getMeta($this->getResourceChannelKey($channel), true);
    }

    /**
     * 设置某资源频道对当前 bot 的开关
     */
    public function setResourceChannel(string $channel, bool $enabled): bool
    {
        $this->wechatBot->setMeta($this->getResourceChannelKey($channel), $enabled);

        return true;
    }

    /**
     * 获取所有已显式配置的资源频道开关（用于展示）
     *
     * @return array<string, bool>
     */
    public function getResourceChannels(): array
    {
        $channels = [];

        foreach ($this->wechatBot->meta as $key => $meta) {
            if (preg_match('/^keyword_resources_(.+)_enabled$/', $key, $m)) {
                $channels[$m[1]] = (bool) $meta->value;
            }
        }
